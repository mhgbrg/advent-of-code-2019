...
func main() {
	...
	discover(machine, grid.Position{X: 0, Y: 0}, discovered)
	...
	oxygenPos := grid.Position{X: 16, Y: -12}
	minutes := fill(discovered, oxygenPos, make(map[grid.Position]bool)) - 1
	fmt.Println(minutes)

	fmt.Println(time.Since(start))
}

func discover(machine *Machine, pos grid.Position, discovered map[grid.Position]int) {
	for direction := 1; direction <= 4; direction++ {
		...
		discover(machine, destination, discovered)
...
func fill(area map[grid.Position]int, pos grid.Position, filled map[grid.Position]bool) int {
	if area[pos] == wall || filled[pos] {
		return 0
	}
	filled[pos] = true
	maxMinutes := math.MinInt32
	for direction := 1; direction <= 4; direction++ {
		minutes := fill(area, pos.Move(direction), filled)
